...
export const FirebaseProvider: React.FC<FirebaseProviderProps> = ({
  ...
  useEffect(() => {
    ...
    const authUnsubscribe = onAuthStateChanged(
      auth,
      (firebaseUser) => {
        profileUnsubscribe?.(); // Unsubscribe from old profile listener

        if (firebaseUser) {
          // User is signed in, but we are still loading until we get the profile
          setUserAuthState(prevState => ({ ...prevState, user: firebaseUser, profile: null, isUserLoading: true }));
          ...
          profileUnsubscribe = onSnapshot(
            ...
            (profileSnap) => {
                if (profileSnap.exists()) {
                    const profileData = { ...(profileSnap.data() as Omit<UserProfile, 'id'>), id: profileSnap.id };
                    setUserAuthState({
                        user: firebaseUser,
                        profile: profileData,
                        isUserLoading: false, // Now we are done loading
                        userError: null,
                    });
                } else {
                    // This case handles when the user is authenticated but their profile document has been deleted.
                    // This can happen if an admin deletes the user. We should treat them as logged out.
                    auth.signOut();
                }
            },
            (error) => {
          ...
          // User is signed out, not loading
